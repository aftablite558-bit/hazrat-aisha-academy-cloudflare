              ...
              return (
                <tr key={f.id}>
                  ...
                  <td>
                    ...
                    <div className="text-xs text-muted-foreground">Class: {classes.find(c => c.id === f.classId)?.className || f.classId}</div>
                  </td>
                  <td>{f.dueDate}</td>
      ...
      <GlassModal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={selectedFee ? "Edit Fee Record" : "Collect Fee"} className="max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <GlassSelect required label="Class" value={formData.classId} onChange={e => setFormData({...formData, classId: e.target.value})} options={[{label:"Select Class", value:""}, ...classes.map(c => ({label: c.className || (c as any).name, value: c.id}))]} />
